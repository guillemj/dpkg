...
test_strvec_parts(void)
{
	...
	sv = strvec_split("", ':', STRVEC_SPLIT_NONE);
	test_pass(sv != NULL);
	test_pass(sv->used == 1);
	test_str(sv->vec[0], ==, "");
	str = strvec_join(sv, '/');
	...
	sv = strvec_split("one", ':', STRVEC_SPLIT_NONE);
	test_pass(sv != NULL);
	test_pass(sv->used == 1);
	...
	sv = strvec_split(":end", ':', STRVEC_SPLIT_NONE);
	test_pass(sv != NULL);
	test_pass(sv->used == 2);
	test_str(sv->vec[0], ==, "");
	test_str(sv->vec[1], ==, "end");
	...
	sv = strvec_split("ini:", ':', STRVEC_SPLIT_NONE);
	test_pass(sv != NULL);
	test_pass(sv->used == 2);
	test_str(sv->vec[0], ==, "ini");
	test_str(sv->vec[1], ==, "");
	...
	sv = strvec_split("foo:bar:quux:foo::end:", ':', STRVEC_SPLIT_NONE);
	test_pass(sv != NULL);
	test_pass(sv->used == 7);
