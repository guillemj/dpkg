...
int
packagelist::compareentries(const struct perpackagestate *a,
                            const struct perpackagestate *b)
{
	switch (statsortorder) {
	case sso_avail:
		if (a->ssavail != b->ssavail)
			return a->ssavail - b->ssavail;
		break;
	case sso_state:
		if (a->ssstate != b->ssstate)
			return a->ssstate - b->ssstate;
		break;
	case sso_unsorted:
		break;
	default:
	...
	if (!bsection && b->pkg->set->name)
		bsection = "";
	// TODO: Refactor these overly complex assignments.
	int c_section = !asection || !bsection ?
	                    (!bsection) - (!asection) :
	                    !*asection || !*bsection ?
	                        (!*asection) - (!*bsection) :
	                        strcasecmp(asection, bsection);
	int c_priority = a->pkg->priority - b->pkg->priority;
	if (!c_priority && a->pkg->priority == PKG_PRIO_OTHER)
		c_priority = strcasecmp(a->pkg->otherpriority, b->pkg->otherpriority);
	int c_name = a->pkg->set->name && b->pkg->set->name ?
...
void
packagelist::addheading(enum ssavailval ssavail,
...
int
qsort_compareentries(const void *a, const void *b)
{
...
packagelist::ensurestatsortinfo()
{
	...
	case sso_unsorted:
		debug(dbg_general,
		      "packagelist[%p]::ensurestatsortinfos() unsorted",
		      this);
		return;
	case sso_avail:
		debug(dbg_general,
		...
		for (index = 0; index < nitems; index++) {
			debug(dbg_general,
			      ...
			      this, index,
			      pkg_name(table[index]->pkg, pnaw_always));
			pkg = table[index]->pkg;
			switch (pkg->status) {
	...
	case sso_state:
		...
		for (index = 0; index < nitems; index++) {
			...
			case PKG_STAT_INSTALLED:
				...
				break;
			default:
				internerr("unknown status %d on sso_state",
...
packagelist::sortmakeheads()
{
	...
	if (sortorder == so_alpha && statsortorder == sso_unsorted) {
		sortinplace();
		return;
	}
	...
	for (a = 0; a < nrealitems; a++) {
		...
		if (thispkg->set->name == nullptr)
			internerr("package set has no name at table index %d",
			          a);
		...
		debug(dbg_general,
		      "packagelist[%p]::sortmakeheads() pkg=%s  "
		      "state=%d avail=%d %s  "
		      "priority=%d otherpriority=%s %s  "
		      "section=%s %s",
...
packagelist::initialsetup()
{
	...
	// Will realloc if necessary, so 150 not critical.
	nallocated = allpackages + 150;
	...
	while ((pkg = pkg_hash_iter_next_pkg(iter))) {
		...
		if (modstatdb_get_status() == msdbrw_write &&
			...
			state->suggested =
			    ...
			    pkg->priority <= PKG_PRIO_STANDARD ?
...
packagelist::matchsearch(int index)
{
	...
	name = itemname(index);
	if (!name)
		// Skip things without a name (separators).
		return false;
...
/*
 * Returns list of packages as nullptr-terminated array, which becomes owned
 * by the caller, if a recursive check is desired.
 * Returns nullptr if no recursive check is desired.
 */
pkginfo **
packagelist::display()
{
	...
	if (recursive) {
		...
		debug(dbg_general,
		      "packagelist[%p]::display() done, not recursive no retl",
		      this);
