...
baselist::startdisplay()
{
	...
	if (has_colors() &&
		...
		/* User defined attributes for B&W mode are not currently
		 * supported. */
...
baselist::redrawall()
{
	...
	mywerase(listpad);
	// Start is first drawn; end is first undrawn; -1 = none.
...
void
baselist::redraw1item(int index)
{
...
void
baselist::refreshinfo()
{
	pnoutrefresh(infopad, infotopofscreen, leftofscreen, info_row, 0,
	             min(info_row + info_height - 1,
	                 info_row + MAX_DISPLAY_INFO - 1),
	...
	if (whatinfo_height) {
		...
		if (infolines > info_height) {
			...
			if (infotopofscreen + info_height < infolines) {
				wprintw(whatinfowin, _("%s for more"),
				        bindings->find("iscrollon").str());
				if (infotopofscreen)
					waddstr(whatinfowin, ", ");
			}
			if (infotopofscreen)
				wprintw(whatinfowin, _("%s to go back"),
				        bindings->find("iscrollback").str());
			waddch(whatinfowin, '.');
		}
...
void
baselist::wordwrapinfo(int offset, const char *m)
{
	ssize_t usemax = xmax - 5;
	debug(dbg_general,
	      "baselist[%p]::wordwrapinfo(%d, '%s')", this, offset, m);
	...
	for (;;) {
		...
		if (!l || (*m == '.' && l == 1)) {
			...
			for (;;) {
				...
				if (dosend)
					waddnstr(infopad, m, dosend);
				while (dosend < l && m[dosend] == ' ')
					dosend++;
				l -= dosend;
				m += dosend;
				if (l <= 0)
