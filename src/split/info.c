...
static char *
nextline(char **ripp, const char *fn, const char *what)
{
	char *newline, *rip;
...
read_info(struct dpkg_ar *ar, struct partinfo *ir)
{
	...
	ir->version = nfstrsave(nextline(&rip, ar->name,
	                                 _("package version number")));
	ir->md5sum = nfstrsave(nextline(&rip, ar->name,
	...
	ir->maxpartlen = parse_intmax(nextline(&rip, ar->name,
	...
	*slash++ = '\0';
	 ...
	 * the architecture. */
	if (*rip != '\0')
		ir->arch = nfstrsave(nextline(&rip, ar->name,
