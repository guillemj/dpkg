...
cu_installnew(int argc, void **argv)
{
	...
	if (!(namenode->flags & FNNF_NEW_CONFF) &&
		 ...
		 * remove the backup. */
			...
			ohshite(_("cannot remove backup copy of '%s'"),
			        namenode->name);
	} else if (namenode->flags & FNNF_PLACED_ON_DISK) {
		debug_at(dbg_eachfiledetail, "removing new file");
		if (secure_remove(fnamevb.buf) &&
		    errno != ENOENT && errno != ENOTDIR)
			ohshite(_("cannot remove newly-installed version of '%s'"),
			        namenode->name);
	} else {
	...
	if (secure_remove(fnamenewvb.buf) &&
	    errno != ENOENT && errno != ENOTDIR)
		ohshite(_("cannot remove newly-extracted version of '%s'"),
		        namenode->name);
