            ...
            enum dpkg_tar_options taroption, int admininfo)
{
	...
	if (strcmp(versionbuf, DPKG_AR_MAGIC) == 0) {
		...
		if (sscanf(ctrllenbuf, "%jd%c%d", (intmax_t *)&ctrllennum, &nlc, &dummy) != 2 ||
		    nlc != '\n')
			ohshit(_("archive has malformed control member size '%s'"),
			       ctrllenbuf);
		...
		if (admininfo) {
			...
			if (fd_skip(ar->fd, ctrllennum, &err) < 0)
				ohshit(_("cannot skip archive control member from '%s': %s"),
				       ar->name, err.str);
		}
	...
	if (taroption) {
		...
		if (!c3) {
			...
			if ((taroption & DPKG_TAR_LIST) &&
			    (taroption & DPKG_TAR_EXTRACT))
				...
				internerr("unknown or missing tar action '%d'",
				          taroption);
...
do_raw_extract(const char *const *argv)
{
	...
	extracthalf(debar, control_dir,
	            DPKG_TAR_EXTRACT | DPKG_TAR_CREATE_DIR, 1);
