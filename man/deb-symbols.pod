...
A B<symbols> file must be shipped in the same binary package as the shared
libraries described within.
This is to guarantee that the symbols descriptions match the shared library
being described,
and so that the B<symbols> file can be found from the package owning the
shared library.
