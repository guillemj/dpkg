...
L<dpkg-query(1)>. The list of supported actions can be found later on
...
Configure the package. See B<--configure> for detailed
...
Remove the package, if not already removed. See B<--remove>
...
and complementations. See L<glob(7)> for detailed information about
globbing.
