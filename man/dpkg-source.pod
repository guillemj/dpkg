...
Extract a source package (B<--extract>.
...
and directories had been created - directories and executable
...
Support for long option since dpkg 1.17.14.
...
Support for short option since dpkg 1.14.8.
Support for long option since dpkg 1.15.5.
Support for I<xz> compression since dpkg 1.15.5.
...
filtered out of the list of files for the diff.
(This list is
...
very exhaustive, but if you need to replace it, note that by
...
Support for long option since dpkg 1.15.6.

=item B<--extend-diff-ignore>=I<regex>

The perl regular expression specified will extend the default value used by
B<--diff-ignore> and its current value, if set.
...
signature that can be verified either with the user supplied
B<--signer-certs> keyrings,
or one of the vendor-specific keyrings,
...
upstream signature B<.orig.tar.gz.asc>.
...
Support for upstream signatures during extraction since dpkg 1.18.5.
Support for upstream signatures during building since dpkg 1.19.0.
...
outside of the debian sub-directory.
...
Support for extraction since dpkg 1.13.9.
Support for building since dpkg 1.14.8.
...
B<.orig->I<addon>B<.tar.>I<ext>B<.asc>).

Supported since dpkg 1.14.17.
Support for upstream signatures during extraction since dpkg 1.17.20.
Support for upstream signatures during building since dpkg 1.18.5.
...
automatic patch generated during build.
...
and if there are supplementary original tarballs.
...
=item B<--no-unapply-patches>

=item B<--unapply-patches>
...
Support for long option B<--unapply-patches> since dpkg 1.15.8.
Support for long option B<--no-unapply-patches> since dpkg 1.16.5.
...
For example, to include only the main branch, use C<--git-ref=main>.
To include all tags and
branches, except for the private branch,
use C<--git-ref=--all --git-ref=^private>.
...
=head2 cannot represent changes to I<file>

Changes to upstream sources are usually stored with patch files,
but not all changes can be represented with patches generated by GNU diff:
they can only alter the content of plain text files.
If you try replacing a file with something of a different type
(for example replacing a plain file with a symlink or a directory),
you will get this error message.

You can use «git format-patch» or «git diff --no-index I<dir.orig>/ I<dir>/»
to generate such patches.

=head2 newly created empty file I<file> will not be represented in diff

GNU diff patch files cannot represent creating an empty file.
Thus this change is not recorded in the source package and you are
warned about it.
...
GNU diff patch files do not record permissions of files and thus executable
permissions are not stored in the source package.
This warning reminds you of that fact.
...
GNU diff patch files do not record permissions of files and thus modified
permissions are not stored in the source package.
This warning reminds you of that fact.
