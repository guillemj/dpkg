...
B<--auto>
option to return to the automatic mode (or you can rerun
B<--config>
...
slave alternative link will not be installed (a warning will still
be displayed).
...
(see section L</QUERY FORMAT> below).
...
Supported since version 1.20.1.
...
Specifies the log file, when this is to be different
