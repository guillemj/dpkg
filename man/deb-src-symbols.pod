...
=head3 Using the #CURVER# metavariable
...
Support for tags B<arch-bits> and B<arch-endian> since dpkg 1.18.0.
...
See the L</Standard symbol tags> subsection above for more information.
...
of the real symbol.
To distinguish among different pattern types, a
...
Note that while mangled name is unique in the library by definition,
...
Note that while old style wildcard patterns (denoted by "*@version" in
...
success.
Note, however, that manual reordering of template file entries
