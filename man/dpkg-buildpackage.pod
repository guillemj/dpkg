...
Support for the I<filename>.dsc and I<directory> arguments since dpkg 1.22.7,
with experimental semantics.
...
B<Note>: When using this value standalone and if what you want is to
only (re-)build the source package from a clean source tree, using
...
Equivalent to B<--build=source,all>.

Supported since dpkg 1.17.11.

=item B<-G>

Equivalent to B<--build=source,any>.
...
B<--build=source,any,all>.

Supported since dpkg 1.15.8.

=item B<--target=>I<target>[B<,>...]

=item B<--target> I<target>[B<,>...]

=item B<-T>, B<--rules-target=>I<target>[B<,>...]
...
Support for short option since dpkg 1.15.0.
Support for long option since dpkg 1.18.8.
Support for multiple targets since dpkg 1.18.16.

=item B<--as-root>

Only meaningful together with B<--target>.
Requires that the target be run with root rights.

Supported since dpkg 1.15.0.
...
=item B<--source-by=>I<maintainer-address>

Pass as B<-m> to B<dpkg-genchanges>.
See its manual page.

Support for B<--build-by> since dpkg 1.18.8.
Support for B<--source-by> since dpkg 1.21.10.

=item B<--release-by=>I<maintainer-address>

=item B<--changed-by=>I<maintainer-address>

Pass as B<-e> to B<dpkg-genchanges>.
See its manual page.

Support for B<--release-by> since dpkg 1.18.8.
Support for B<--changed-by> since dpkg 1.21.10.
...
Support for long option since dpkg 1.17.17.

=item B<-t>, B<--host-type> I<gnu-system-type>

Specify the GNU system type we build for.
...
The exclamation marks are not part of the build profile names,
and thus not accepted in this option.

Support for short option since dpkg 1.17.2.
Support for long option since dpkg 1.18.8.
...
Support for short option since dpkg 1.14.7.
Support for long option since dpkg 1.18.8.
...
Support for short option since dpkg 1.18.2.
Support for long option since dpkg 1.18.8.

=item B<--jobs-force>[=I<jobs>|B<auto>]

This option is equivalent to the B<--jobs> option
...
falling back to its legacy default value B<binary-targets>.
...
Clean the source tree before building.
This is the default behavior.

Supported since dpkg 1.18.8.
...
Support for short option since dpkg 1.14.17.
Support for long option since dpkg 1.18.8.
...
which will run at the times specified in the run steps.
The hooks will
...
Set the I<filename> for the generated B<.buildinfo> file.

Supported since dpkg 1.21.0.

=item B<--buildinfo-option=>I<opt>

Pass option I<opt> to B<dpkg-genbuildinfo>.
Can be used multiple times.

Supported since dpkg 1.18.11.
...
Any conforming Stateless OpenPGP (SOP) implementation,
such as sqop, rsop, gosop, hop or pgpainless-cli.
...
default or auto-detected backend command.
...
secret key to use when signing packages.

Support for long option since dpkg 1.21.10.

=item B<--sign-key=>I<key-id>

Deprecated since dpkg 1.23.8, replaced by B<--sign-keyid>.
Supported since dpkg 1.18.8.
...
Do not sign the B<.buildinfo> file.
...
file and the B<.changes> file.

Supported since dpkg 1.18.20.

=item B<--force-sign>

Force the signing of the resulting files, regardless of
B<-us>, B<--unsigned-source>,
B<-ui>, B<--unsigned-buildinfo>,
B<-uc>, B<--unsigned-changes>
or other internal heuristics.

Supported since dpkg 1.17.0.
...
Support for long options since dpkg 1.18.8.

=item B<--source-option=>I<opt>

Pass option I<opt> to B<dpkg-source>.
Can be used multiple times.
...
Set the I<filename> for the generated B<.changes> file.

Supported since dpkg 1.21.0.

=item B<--changes-option=>I<opt>

Pass option I<opt> to B<dpkg-genchanges>.
Can be used multiple times.
...
=item I<$XDG_CONFIG_HOME/dpkg/buildpackage.conf>
...
L<fakeroot(1)>,
L<lintian(1)>,
L<SOP|https://datatracker.ietf.org/doc/draft-dkg-openpgp-stateless-cli/>,
L<sq(1)>,
L<gpg(1)>.
