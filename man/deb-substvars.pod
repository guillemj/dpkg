...
instantiated as a field,
if that field is not defined for B<.dsc> and B<.deb> control files.

The variable name needs to be formed as a namespace followed by
a colon (‘B<:>’) and then the field name,
such as B<tool:Depends>.
All implicit variables for any namespace matching the same field
will get concatenated using a known field separator,
or a space if the field separator is unknown.
...
B<Description> field, if it exists.
