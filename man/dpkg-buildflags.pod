...
B<dpkg-buildflags>: relevant environment variables,
...
and the compiler flags with their origin.
...
Support for B<Flag> stanza since dpkg 1.19.0.
Support for B<Builtins> field in B<Area> stanza since dpkg 1.21.14.

=item B<--query-features> I<area>

Print the features enabled for a given area.
...
The output is in L<deb822(5)> format, with one section per feature.
For example:
...
Support for B<Builtin> field since dpkg 1.21.14.
...
B<Note>: The L<getconf(1)> interface to query LFS build flags,
such as B<LFS_CFLAGS> or B<LFS_LDFLAGS>,
must B<not> be used because this interface does not support cross-building.
...
Supported since dpkg 1.22.0.

=item B<time64>

This setting enables 64-bit time_t support
...
architectures including i386 but not hurd-i386
...
Enabled by default except for i386 and hurd-i386 since dpkg 1.22.5.
...
Enabled by default since dpkg 1.22.6.

Supported since dpkg 1.22.3.

=item B<bug>

This setting adds any warning
...
Supported since dpkg 1.17.4.

=item B<canary>

This setting adds dummy canary
...
This setting adds B<-fstack-protector --param=ssp-buffer-size=4>
...
Enabled by default if B<stackprotectorstrong> is not in use.
...
This setting adds B<-fcf-protection>
...
No global default since dpkg 1.18.23,
as it is enabled by default now by gcc on the
amd64,
arm64,
armel,
armhf,
...
Support for B<DEB_>I<flag>B<_MAINT_SET> since dpkg 1.16.1.
...
Support for B<DEB_>I<flag>B<_MAINT_APPEND> since dpkg 1.16.1.
...
Some build profiles might currently require their counterpart to also
be included in the build options,
the list of such build profiles is vendor specific.
For more information about build profiles,
see L<dpkg-buildpackage(1)>.

Support for B<DEB_BUILD_MAINT_OPTIONS> since dpkg 1.16.1.
...
This variable sets the build path to use in features
...
=item B<$XDG_CONFIG_HOME/dpkg/buildflags.conf>
...
Makefile snippet that loads (and optionally exports) all flags
supported by B<dpkg-buildflags> into variables.
...
(and optionally exports them) into variables.
...
And you can pass the flags to commands manually:
