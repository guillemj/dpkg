...
Support for long command since dpkg 1.17.17.

=item B<-e>, B<--equal> I<architecture>

Check for equality of architecture.
...
Supported since dpkg 1.13.13.
Support for long command since dpkg 1.17.17.

=item B<-i>, B<--is> I<architecture-wildcard>

Check for identity of architecture.
It compares the current or specified Debian host architecture against
...
the specified architecture wildcard.
...
to either B<shell> (default) or B<make>.
...
Example paths: /lib/powerpc64le-linux-gnu/, /usr/lib/i386-gnu/.
...
See the L</TERMS> section for the meanings of these terms.
...
The Debian ABI name of the host machine.

Supported since dpkg 1.18.11.

=item B<DEB_HOST_ARCH_LIBC>

The Debian libc name of the host machine.
...
The Debian architecture of the target machine.
...
The Debian ABI name of the target machine.

Supported since dpkg 1.18.11.

=item B<DEB_TARGET_ARCH_LIBC>

The Debian libc name of the target machine.

Supported since dpkg 1.18.11.

=item B<DEB_TARGET_ARCH_OS>

The Debian system name of the target machine.

Supported since dpkg 1.17.14.

=item B<DEB_TARGET_ARCH_CPU>

The Debian CPU name of the target machine.

Supported since dpkg 1.17.14.

=item B<DEB_TARGET_ARCH_BITS>

The pointer size of the target machine (in bits).

Supported since dpkg 1.17.14.

=item B<DEB_TARGET_ARCH_ENDIAN>

The endianness of the target machine (little / big).

Supported since dpkg 1.17.14.

=item B<DEB_TARGET_GNU_CPU>

The GNU CPU part of B<DEB_TARGET_GNU_TYPE>.

Supported since dpkg 1.17.14.

=item B<DEB_TARGET_GNU_SYSTEM>

The GNU system part of B<DEB_TARGET_GNU_TYPE>.

Supported since dpkg 1.17.14.

=item B<DEB_TARGET_GNU_TYPE>

The GNU system type of the target machine.
...
Format version 1.0, supported since dpkg 1.13.2.

=item I<%PKGDATADIR%/ostable>

Table of known operating system names and mapping to their GNU name.

Format version 2.0, supported since dpkg 1.18.11.
...
Format version 1.0, supported since dpkg 1.18.11.
...
architecture tables are located.
