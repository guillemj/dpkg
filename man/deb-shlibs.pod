...
A B<shlibs> file must be shipped in the same binary package as the shared
libraries described within.
This is to guarantee that the mappings match the shared library
being described,
and so that the B<shlibs> file can be found from the package owning the
shared library.
