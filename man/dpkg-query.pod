...
Note you will probably have to quote I<package-name-pattern> to
...
See L<dpkg(1)> for information about the above states and flags.
...
internal dpkg database, switch to use B<--control-list> and
...
directory to «I<directory>%ADMINDIR%».
...
Support for short option B<-f> since dpkg 1.13.1.
...
It contains the package short description.

Supported since dpkg 1.19.1.

=item B<binary:Summary>

This is an alias for B<binary:Synopsis>.
...
Sets the pager command to use, which will be executed
with «B<$SHELL -c>».
