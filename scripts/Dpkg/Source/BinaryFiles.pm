...
sub detect_binary_files {
    ...
    my $filter_ignore = sub {
        ...
        chdir $self->{dir}
            or syserr(g_("cannot change directory to '%s'"), $self->{dir});
        $exclude{$_} = 1 foreach glob $exclude_glob;
        chdir $cwd or syserr(g_("cannot change directory to '%s'"), $cwd);
        chdir $File::Find::dir
            or syserr(g_("cannot change directory to '%s'"), $File::Find::dir);
        $exclude{$_} = 1 foreach glob $exclude_glob;
        chdir $cwd or syserr(g_("cannot change directory to '%s'"), $cwd);
        my @result;
        foreach my $fn (@_) {
